...
//List of mcnpx itypes encoded in ssw files, corresponding pdg code, and (if
...
int main(){
  ...
  for (unsigned i = 0; i < sizeof(known_mcnpx_mappings)/sizeof(known_mcnpx_mappings[0]); i+=2) {
    ...
    if (mcnpx!=mcnpxconv) {
      printf("Reverse conversion via conv_mcnpx_pdg2ssw failed (got %li)!\n",(long)mcnpxconv);
      return 1;
    }
  }

  for (unsigned i = 0; i < sizeof(known_mcnp6_mappings)/sizeof(known_mcnp6_mappings[0]); i+=3) {
    ...
    if (mcnp6_desiredreverse!=mcnp6conv) {
      printf("Reverse conversion via conv_mcnp6_pdg2ssw failed (got %li, wanted %li)!\n",(long)mcnp6conv,(long)mcnp6_desiredreverse);
      return 1;
    }
  }
